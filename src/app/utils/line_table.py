...
class LineTable():
    ...
    def __init__(self, rows, width=27):
        self.rows = rows
        self.width = width
...
class BlockTable():
    """This type of table has optional headers and uses a discord code block for 2 or 3 columns of data"""

    def __init__(self, columns, rows, width=27):
        ...
        for row in self.rows:
            ...
            for i in range(columns):
                if len(row[i]) > column_widths[i]:
                    row[i] = self._truncate_cell(row[i], column_widths[i])
            str_rows.append(" ".join([f"{row[i]}".ljust(column_widths[i]) for i in range(columns)]))
        headers = " ".join([f"{self.columns[i]}".ljust(column_widths[i]) for i in range(columns)])
        rows_per_table = int(2000/(self.width+columns))
        _tables = []
        for i in range(0, len(self.rows), rows_per_table):
            start = i
            end = i + rows_per_table - 2
            content = "\n".join([f"{row}" for row in str_rows[start:end]])
            divider = "-"*len(headers)
            _table = f"```{headers}\n{divider}\n{content}```"
